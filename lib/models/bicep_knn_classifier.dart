...
class BicepKNNClassifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        rootBundle.loadString('assets/json/bicep_scaler.json'),
        rootBundle.loadString('assets/json/bicep_label_encoder.json'),
        rootBundle.loadString('assets/json/bicep_knn_data.json'),
      ]);
      ...
      if (kDebugMode) {
        print(
          '✅ Bicep KNN Classifier initialized in ${stopwatch.elapsedMilliseconds}ms',
        );
  ...
  Map<String, dynamic> predict(List<double> features) {
    if (!_isReady || features.length != 7) {
      return {'label': 'neutral', 'confidence': 0.0, 'label_index': -1};
    }
  ...
  List<double> _standardizeFeatures(List<double> features) {
    ...
    for (int i = 0; i < features.length; i++) {
      final scaled =
          (features[i] - _scalerMean![i]) / (_scalerScale![i] + 1e-6);

...
import './shoulderpress-feature-extract.dart';
...
class ShoulderPressClassifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      final labelsJson = await rootBundle.loadString('assets/models/label_encoder.json');
      ...
      final modelBytes = await rootBundle.load('assets/models/shoulder_press_model.onnx');
  ...
  Future<Map<String, dynamic>> predict(Pose pose) async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('[Prediction] Output indices: ${List<int>.generate(outputs.length, (i) => i)}');
      }
      ...
      if (probOutput != null) {
        if (probOutput is Map) {
          ...
          probabilities = (probOutput[0] as List)
      ...
      final confidence = probabilities.isNotEmpty 
          ? probabilities[predictedClassIdx] 
          : 1.0;
      ...
      if (kDebugMode) {
        ...
        print('║  Confidence: ${(confidence * 100).toStringAsFixed(1).padRight(43)}% ║');
